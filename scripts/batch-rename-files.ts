...
const renamings = {
  prepend: (filenames, extensions, prefix) =>
    filenames.map((file, i) => `${prefix}${file}.${extensions[i]}`),
  append: (filenames, extensions, suffix) =>
    filenames.map((file, i) => `${file}${suffix}.${extensions[i]}`),
  replaceWithRegex: (filenames, extensions, regexParts) => {
    const [search, replace] = regexParts.split('|');
    return filenames.map(
      (file, i) => `${file.replace(new RegExp(search, 'g'), replace)}.${extensions[i]}`,
    );
  },
  generateNumberedList: (filenames, extensions, index = 0) =>
    filenames.map((file, i) => `${index + i + 1}_${file}.${extensions[i]}`),
};
