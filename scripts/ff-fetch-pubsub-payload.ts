...
if (operationType === 'service') {
  ...
  const topics = await cache.remember(`topics.${serviceName}`, async () => {
    ...
    const { stdout: subscriptions } = await exec(
      `${gcloud} pubsub subscriptions list --project=${env} --format=json | \
      ${jq} "[.[] | select(.pushConfig.oidcToken.audience == \\"${serviceUrl}\\")]"`,
    );
  ...
  topic = await arg(
    'Choose a topic',
    [...topics, cache.defaultInvalidate].map((topic) => {
  ...
  const topics = await cache.remember(`topics`, async () => {
    const { stdout: reponse } = await exec(
      `${gcloud} pubsub subscriptions list --project=${env} --format=json`,
    );
    ...
    const topics = subscriptions
      .map((subscription) => {
        ...
        try {
          const serviceName = subscription.pushConfig.oidcToken.audience.replace(
            /^https:\/\/(.+?)-\w{10}-uc\.a\.run\.app$/,
            '$1',
          );
          return {
  ...
  const selectedTopic: Record<string, string> | 'invalidate' = await arg('Choose a topic', [
    ...topics,
...
const payloads = await cache.remember(
  `payloads.${serviceName}.${topic}`,
  async () => {
    let payloads = [];
    ...
    do {
      ...
      const { stdout: response } = await exec(
        `${gcloud} logging read 'resource.labels.service_name="${serviceName}" AND 
        severity="DEBUG" AND
        jsonPayload.body.subscription="${topic}" AND 
        timestamp>="'$(${date} -d '-1 ${dateUnit}' --iso-8601=seconds --utc)'"' \
        --limit=20 \
        --format=json \
        --project=${env} | \
        ${jq} '[.[] | .jsonPayload.body]'`,
...
const payload = await arg(
  ...
  [cache.defaultInvalidate, ...payloads].map((payload) => {
    ...
    const name = !invalidate
      ? `Payload from ${dayjs(payload.message.publishTime).format('MMM D, YYYY h:mm A')}`
      : payload.name;
    return {
      name,
      preview: () => {
        ...
        const pretty = JSON.stringify(JSON.parse(text), null, 2).replaceAll('\\n', '\n');
