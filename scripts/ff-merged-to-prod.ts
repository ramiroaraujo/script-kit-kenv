...
for(let prURL of prURLs) {
    ...
    const prSha = await exec(`cd ${path} && ${gh} pr view ${prNumber} --json commits --jq '.commits[-1].oid'`);
    await exec(`cd ${path} && git fetch origin prod > /dev/null 2>&1`);
    const isMerged = (await exec(`cd ${path} && git merge-base --is-ancestor ${prSha.stdout.trim()} origin/prod && echo "yes" || echo "no"`)).stdout.trim();
