...
const envLines = Object.entries(envs).map(([key, value]) => `<env name="${key}" value="${value}" />`).join("\n      ");
...
await ensureReadFile(`${path}/.idea/runConfigurations/_template__of_JavaScriptTestRunnerJest.xml`, file, 'utf-8');
