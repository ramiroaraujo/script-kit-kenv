  ...
  },
  {
    ...
    value: {
      ...
      collections: [
        ...
        'jb_user_token_recent_applications',
        'jd_bid_log',
...
if (url.type === 'run') {
  ...
  finalUrl = await arg('Choose a Cloud Run instance', [openList, ...instances]);
} else if (url.type === 'scheduler') {
  const data = await cache.remember('scheduler', async () => {
    const cloudSchedulerInstances = await exec(
      `${gcloud} scheduler jobs list --project=${env} --format="json"`,
    );
  ...
  const result = await arg('Choose a Cloud Scheduler task', [openList, ...schedulerInstances]);
  if (result !== 'Open') {
  ...
  finalUrl = await arg('Choose a Storage bucket', [openList, ...buckets]);
} else if (url.type === 'secrets') {
  const data = await cache.remember('secrets', async () => {
    const secretsOutput = await exec(`${gcloud} secrets list --project=${env} --format="json"`);
    return JSON.parse(secretsOutput.stdout);
  });

  const secrets = data.map((secret) => ({
    name: `Open secret ${secret.name.split('/').pop()}`,
    value: `${cloudUrl}/security/secret-manager/secret/${secret.name
      .split('/')
      .pop()}?project=${env}`,
  }));

  finalUrl = await arg('Choose a Secret', [openList, ...secrets]);
